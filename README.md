...
| 出所 | 数 | 内容 |
|---|---|---|
| 標識令の図案 | 44 | 別表第二（標識）の図案SVG（`signs/`）を 64×64 に正規化してそのまま使う |
| 道路標示の図にもとづく作図 | 25 | 対応する標識が無い路面標示など。**別表第六（道路標示）の図を e-Gov で見て**形を決めている |
| 標識の図案にもとづく作図 | 1 | `70` 駐車可。標識403の意匠（青地に白のP）だが Commons に再現SVGが無い |
...
図案を使わず手で描いているのは次の2つの理由で、コードごとの根拠は
...
手描きのものも、**対応する道路標示があるものは実物の図を見て描いています**
（25コード）。図は e-Gov の法令データから取り出しています（寸法線入りの JPEG なので
参照にだけ使用）。標示番号は [`data/handdrawn_reasons.csv`](data/handdrawn_reasons.csv)
と [`docs/icon-list.md`](docs/icon-list.md) の「参考」列にあります。
何も参照せずゼロから描いているのは `94` `98` `102` の3コードだけです。

**手描きの29コードは精査中です。** 実物と違うまま残っているもの（路面の文字を
絵で代えている `21` `24` `110` `111`、3種の標示を1つにまとめている `116`、
JARTIC 独自の組合せ `107` `118` `119`）と、地図上で読めるか確かめていないものを
[issue #1](https://github.com/shiwaku/jartic-regulation-sprite/issues/1) に列挙しています。
...
```bash
npm install
npm run icons     # signs/ の取り込み＋手描き分の生成で icons/*.svg を作る
npm run verify    # コード表・SVG・スプライトの整合を見る
npm run bounds    # 手描き分が枠や台紙からはみ出していないか調べる
...
**ツール類・手描きのアイコン**: MIT License（[LICENSE](LICENSE)）。
